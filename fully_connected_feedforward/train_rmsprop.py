...
from mnist_lib.train import train_rmsprop;
...
if __name__ == '__main__':
    ...
    t1 = time.time();
    for ep, loss in train_rmsprop(train_model, data, N_EPOCHS,
                                  EPOCH_SIZE, learn_rate_schedule,
                                  momentum_schedule, CLIP_VAL,
                                  decay=.9):
        t = time.time() - t1;
        if ep % 10 == 0:
    ...
    print('Saving model...');
    test_model.save_weights('models/rmsprop.vars')
    with open('models/rmsprop.pkl', 'w') as f:
